<template>
  ...
  <song-list
    v-if="detail.playlist"
    :collect="detail.playlist.subscribedCount"
...
import { SongSheetDetail } from "../api/data.d";
...
export default defineComponent({
  ...
  setup() {
    ...
    const updateData = () => {
      if (query && query.id) {
        playlistDetail({ id: +query.id }).then((res) => {
          info.detail = res;
        });
      } else {
        router.push("/index");
    ...
    const handlePlay = (id: Event) => {
      ...
      store.commit("MutateCurrentPos", {
        curPos: tracks.findIndex(
          (item) => item.id === (id as unknown as number)
        ),
      });
      store.dispatch("ActSongUrls", {
        id: tracks.map((item) => item.id).join(","),
      });
    };
    return {
      ...toRefs(info),
